...
start=$(date +"%s")
echo ""
echo ">>> Just for pipeline_opt Compiling the object files and Generating the optimized HLS C code..."
echo ""
cd build
N_values=(32 64 128 256 512 1024 2048 4096 8192)
# N_values=(32 64 256 1024 2048 4096)

# N_values=(32)
# targets=("2mm.cpp" "3mm.cpp" "gemm.cpp" "bicg.cpp" "gesummv.cpp")
targets=("gemm.cpp" "bicg.cpp")
examples=("gemm" "bicg")
execute_command() {
    source_file=$1
    N=$2
    ...
    # 编译和运行命令
    cmake --build . --target ${source_file_no_extension}
    start_time=$(date +%s.%N)
    ./bin/${source_file_no_extension}
    
    # 使用没有扩展名的路径进行文件操作
    mlir_file="../samples/${source_file_no_extension}/test_${source_file_no_extension}_${N}.mlir"
    cpp_file="../samples/${source_file_no_extension}/test_${source_file_no_extension}_${N}.cpp"
    
    # 进行 scalehls-opt 和 scalehls-translate 操作
    ../scalehls/build/bin/scalehls-opt $mlir_file \
        --scalehls-func-preprocess="top-func=test_${source_file_no_extension}_${N}" \
        --cse -canonicalize \
    ...
    echo "time[${source_file_no_extension},$N] $execution_time" >> execution_times.txt
    echo ""
    echo "The HLS C code of $source_file_no_extension: test_${source_file_no_extension}_${N}.cpp is generated!"
    echo ""
}
...
echo ">>> Step 1 Total Execution Time: $execution seconds"
echo ""
echo ""
...
# echo "[3/3] 执行替换：II=1 -> II=4（仅 pipeline 的 pragma 行），并生成 .bak 备份"
# find "$TARGET_DIR" -type f -name '*.cpp' -exec \
#   sed -i.bak -E '/^[[:space:]]*#pragma[[:space:]]+HLS[[:space:]]+pipeline/ s/\bII[[:space:]]*=[[:space:]]*1\b/II=4/g' {} +
...
echo ">>> Step 2. Synthesising the optimized HLS C code..."
echo ""
cd ..
...
# baseline_latency["3mm",32]= 4198
# baseline_latency["3mm",64]= 33061
# baseline_latency["3mm",256]= 2101287
# baseline_latency["3mm",1024]= 134283303
# baseline_latency["3mm",2048]= 1074004007
