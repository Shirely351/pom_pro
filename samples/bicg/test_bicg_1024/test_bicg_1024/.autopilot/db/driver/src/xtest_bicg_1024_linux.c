...
typedef struct {
    ...
    XTest_bicg_1024_uio_map maps[ MAX_UIO_MAPS ];
} XTest_bicg_1024_uio_info;

/***************** Variable Definitions **************************************/
static XTest_bicg_1024_uio_info uio_info;
...
static int uio_info_read_name(XTest_bicg_1024_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XTest_bicg_1024_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XTest_bicg_1024_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XTest_bicg_1024_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XTest_bicg_1024_Initialize(XTest_bicg_1024 *InstancePtr, const char* InstanceName) {
	XTest_bicg_1024_uio_info *InfoPtr = &uio_info;
